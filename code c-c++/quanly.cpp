...
void tao() { // them nhan vien
    ...
    while (i <= n) {
        ...
        if (kk==-1) 
		{
			a.push_back(nhansu(name, que, d, m, y, email, i+1, vtcv, ml, ds, ms, ys, dt, hv, da, "nhan vien", "-", ml));
            adn.push_back(dnhap(20240000+i+1, "123456"));
            r.push_back(req(0));
            mm++;
        }
        else
